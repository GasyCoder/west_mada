    ...
    <div class="container">
        ...
        <ul class="nav navbar-nav order-2 ms-auto nav-no-dark">
            ...
            @foreach ($navbars as $menu)
                <li class="nav-item @if($menu->dropdown == true) navbar-dropdown @endif">
                    <a class="nav-link" href="#">
                        <span>{{ $menu->name }}</span>
                    </a>
                    <div class="dropdown-menu rounded-2 shadow">
                        <ul class="nav navbar-nav">
                            @foreach ($menu->onglets as $onglet)
                                @foreach ($onglet->contents as $contente)  
                                <li class="nav-item">
                                    <a class="nav-link" href="{{ route('contente', ['slug' => $contente->slug]) }}"
                                        wire:navigate>
                                        <span>{{ $contente->onglet->name }}</span> {{-- Affiche le nom de l'onglet --}}
                                    </a>
                                </li>
                                @endforeach
                            @endforeach
            ...
            <!-- Button-->
            <a class="btn d-none d-sm-inline-flex btn btn-sm btn-accent-3 ms-auto ms-sm-60 me-30 me-sm-0 order-2 order-sm-3"
                href="#" target="_self">
                Contact
                <svg class="ms-10 align-self-center" xmlns="http://www.w3.org/2000/svg" fill="none" viewbox="0 0 15 15">
